<script>
    ...
    function submit() {
        if (canEditTechnicalCommittees || isSuperAdmin) {
            $form.put(
                route('technical-committees.update', technicalCommittee.id),
                {
    ...
    function destroy() {
        if (canDeleteTechnicalCommittees || isSuperAdmin) {
            $form.delete(
                route('technical-committees.destroy', technicalCommittee.id),
            )
        }
    }
</script>
...
<AuthenticatedLayout>
    ...
    <div class="bg-white rounded shadow max-w-3xl">
        <form on:submit|preventDefault={submit}>
            <fieldset
                class="p-8"
                disabled={canEditTechnicalCommittees || isSuperAdmin
                    ? undefined
                    : true}
            >
                <div class="mt-4">
            ...
            >
                {#if canDeleteTechnicalCommittees || isSuperAdmin}
                    ...
                    >
                        {$_('Delete')}
                        {$_('Technical committees.singular').toLowerCase()}
                    </button>
                {/if}
                {#if canEditTechnicalCommittees || isSuperAdmin}
                    <LoadingButton
