<script>
    ...
    function submit() {
        if (canEditStrategicThematics || isSuperAdmin) {
            $form.put(
                route('strategic-thematics.update', strategicThematic.id),
                {
    ...
    function destroy() {
        if (canDeleteStrategicThematics || isSuperAdmin) {
            $form.delete(
                route('strategic-thematics.destroy', strategicThematic.id),
            )
        }
    }
</script>
...
<AuthenticatedLayout>
    ...
    <div class="bg-white rounded shadow max-w-3xl">
        <form on:submit|preventDefault={submit}>
            <fieldset
                class="p-8"
                disabled={canEditStrategicThematics || isSuperAdmin
                    ? undefined
                    : true}
            >
                <div class="mt-4">
