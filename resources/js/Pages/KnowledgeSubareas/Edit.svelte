<script>
    ...
    let form = useForm({
        ...
        knowledge_area_id: {
            value: knowledgeSubarea.knowledge_area_id,
            label: knowledgeAreas.find(
                (item) => item.value == knowledgeSubarea.knowledge_area_id,
            )?.label,
        },
    })
    ...
    function submit() {
        if (canEditKnowledgeSubareas || isSuperAdmin) {
            $form.put(route('knowledge-subareas.update', knowledgeSubarea.id), {
    ...
    function destroy() {
        if (canDeleteKnowledgeSubareas || isSuperAdmin) {
            $form.delete(
                route('knowledge-subareas.destroy', knowledgeSubarea.id),
            )
        }
    }
</script>
...
<AuthenticatedLayout>
    <header class="shadow bg-white" slot="header">
        ...
        >
            <div>
                <h1>
                    {#if canIndexKnowledgeSubareas || canShowKnowledgeSubareas || canEditKnowledgeSubareas || canDeleteKnowledgeSubareas || isSuperAdmin}
                        <a
                            use:inertia
    ...
    <div class="bg-white rounded shadow max-w-3xl">
        <form on:submit|preventDefault={submit}>
            <fieldset
                class="p-8"
                disabled={canEditKnowledgeSubareas || isSuperAdmin
                    ? undefined
                    : true}
            >
                <div class="mt-4">
            ...
            >
                {#if canDeleteKnowledgeSubareas || isSuperAdmin}
                    ...
                    >
                        {$_('Delete')}
                        {$_('Knowledge subareas.singular').toLowerCase()}
                    </button>
                {/if}
                {#if canEditKnowledgeSubareas || isSuperAdmin}
                    <LoadingButton
