...
<script>
    ...
    $title = 'Programs académicos'

    let canCreateAcademicProgram = $page.props.auth.user.can.find(element => element == 'academic-programs.create') == 'academic-programs.create'

    let filters = {}
</script>

<h1 class="mb-8 font-bold text-3xl">{$_("Academic programs")}</h1>
<div class="mb-6 flex justify-between items-center">
    <!-- <SearchFilter class="w-full max-w-md mr-4" bind:filters /> -->
    {#if canCreateAcademicProgram}
        <Link href={route('academic-programs.create')} class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:shadow-outline-gray transition ease-in-out duration-150 btn-indigo ml-auto">
            <div>
                <span>Crear</span>
                <span class="hidden md:inline">programa de formación</span>
            </div>
        </Link>
    {/if}
</div>
