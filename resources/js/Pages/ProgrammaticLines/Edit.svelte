<script>
    ...
    let form = useForm({
        ...
        project_category: {
            value: programmaticLine.project_category,
            label: projectCategories.find(
                (item) => item.value == programmaticLine.project_category,
            )?.label,
        },
    })
    ...
    function destroy() {
        if (canDeleteProgrammaticLines || isSuperAdmin) {
            $form.delete(
                route('programmatic-lines.destroy', programmaticLine.id),
            )
        }
    }
</script>
...
<AuthenticatedLayout>
    ...
    <div class="bg-white rounded shadow max-w-3xl">
        <form on:submit|preventDefault={submit}>
            <fieldset
                class="p-8"
                disabled={canEditProgrammaticLines || isSuperAdmin
                    ? undefined
                    : true}
            >
                <div class="mt-4">
            ...
            >
                {#if canDeleteProgrammaticLines || isSuperAdmin}
                    ...
                    >
                        {$_('Delete')}
                        {$_('Programmatic lines.singular').toLowerCase()}
                    </button>
                {/if}
                {#if canEditProgrammaticLines || isSuperAdmin}
                    <LoadingButton
