<script>
    ...
    $: $title =
        $_('Create') +
        ' ' +
        $_('Partner organization members.singular').toLowerCase()
    ...
    function submit() {
        if (canCreatePartnerOrganizationMembers || isSuperAdmin) {
            $form.post(
                route(
                    'calls.rdi.partner-organizations.partner-organization-members.store',
...
<AuthenticatedLayout>
    ...
    <div class="bg-white rounded shadow max-w-3xl">
        <form on:submit|preventDefault={submit}>
            <fieldset
                class="p-8"
                disabled={canCreatePartnerOrganizationMembers || isSuperAdmin
                    ? undefined
                    : true}
            >
                <div class="mt-4">
