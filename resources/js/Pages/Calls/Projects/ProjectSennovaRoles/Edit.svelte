<script>
    ...
    $: if (
        projectSennovaRoleInfo?.qty_months != null &&
    ...
    function submit() {
        if (canEditProjectSennovaRoles || isSuperAdmin) {
            $form.put(
                route('calls.projects.project-sennova-roles.update', [
                    call.id,
                    project.id,
    ...
    function destroy() {
        if (canDeleteProjectSennovaRoles || isSuperAdmin) {
            $form.delete(
                route('calls.projects.project-sennova-roles.destroy', [
                    call.id,
                    project.id,
                    projectSennovaRole.id,
                ]),
...
<AuthenticatedLayout>
    ...
    <div class="bg-white rounded shadow max-w-3xl">
        <form on:submit|preventDefault={submit}>
            <fieldset
                class="p-8"
                disabled={canEditProjectSennovaRoles || isSuperAdmin
                    ? undefined
                    : true}
            >
                <div class="mt-4">
