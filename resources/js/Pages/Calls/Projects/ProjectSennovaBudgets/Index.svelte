...
<AuthenticatedLayout>
    ...
    <DataTable class="mt-20">
        ...
        <h2 class="text-center mt-10 mb-24" slot="caption">
            Ingrese cada uno de los rubros que requiere el proyecto. Actualmente
            el total del costo de los productos o servicios requeridos es: ${new Intl.NumberFormat(
                'de-DE',
            ).format(
                !isNaN(project.total_project_budget)
        ...
        <tbody slot="tbody">
            <tr class="hover:bg-gray-100 focus-within:bg-gray-100">
                ...
                <td class="border-t p-4">
                    El valor no debe superar el 5% (${new Intl.NumberFormat(
                        'de-DE',
                    ).format(
                        !isNaN(project.total_industrial_machinery)
                            ? project.total_industrial_machinery * 0.05
                            : 0,
                    )}) del rubro de "MAQUINARIA INDUSTRIAL" (${new Intl.NumberFormat(
                        'de-DE',
                    ).format(
                        !isNaN(project.total_industrial_machinery)
                            ? project.total_industrial_machinery
                            : 0,
                    )}).
                </td>
                <td class="border-t p-4">
                    Valor actual: ${new Intl.NumberFormat('de-DE').format(
                        !isNaN(project.total_special_construction_services)
                            ? project.total_special_construction_services
                            : 0,
                    )}
                    {#if project.total_special_construction_services <= project.total_industrial_machinery * 0.05}
                        <span
                            class="bg-green-100 text-green-400 hover:bg-green-200 px-2 py-1 rounded-3xl text-center"
                        >
                            Correcto
                        </span>
                    {:else}
                        <span
                            class="bg-red-100 text-red-400 hover:bg-red-200 px-2 py-1 rounded-3xl text-center"
                        >
                            Incorrecto
                        </span>
                    {/if}
            ...
            <tr class="hover:bg-gray-100 focus-within:bg-gray-100">
                ...
                <td class="border-t p-4">
                    La sumatoria de todos los rubros de viáticos no debe superar
                    el valor de $4.000.000
                </td>
                <td class="border-t p-4">
                    Valor actual: ${new Intl.NumberFormat('de-DE').format(
                        !isNaN(project.total_viatics)
                            ? project.total_viatics
                            : 0,
                    )}
            ...
            <tr class="hover:bg-gray-100 focus-within:bg-gray-100">
                ...
                <td class="border-t p-4">
                    El valor no debe superar el 5% (${new Intl.NumberFormat(
                        ...
                        !isNaN(project.total_project_budget)
                            ? project.total_project_budget * 0.05
                            : 0,
                    )}) del total del proyecto ( ${new Intl.NumberFormat(
                        'de-DE',
                    ).format(
                        !isNaN(project.total_project_budget)
                            ? project.total_project_budget
                ...
                <td class="border-t p-4">
                    Valor actual: ${new Intl.NumberFormat('de-DE').format(
                        !isNaN(project.total_machinery_maintenance)
                            ? project.total_machinery_maintenance
                            : 0,
                    )}
        ...
        <tfoot>
            <tr>
                <td colspan="3">
                    <strong>Nota:</strong> Los valores en paréntesis son los valores
        ...
        <ul class="flex flex-wrap">
            {#each secondBudgetInfo as { name }}
                <li class="mr-2 mb-2 inline-flex">
                    <a
                        class="bg-indigo-100 hover:bg-indigo-200 px-2 py-1 rounded-3xl text-center text-indigo-400"
            ...
            <li class="mr-2 mb-2 inline-flex">
                <a
                    class="bg-green-100 hover:bg-green-200 px-2 py-1 rounded-3xl text-center text-green-400"
                    use:inertia={{ preserveScroll: true }}
                    href="?search=">Todos</a
                >
            </li>
    ...
    <DataTable class="mt-20">
            ...
            {#if canCreateProjectSennovaBudgets || isSuperAdmin}
                <Button
                    on:click={() =>
                        Inertia.visit(
                            route(
                                'calls.projects.project-sennova-budgets.create',
                                [call.id, project.id],
                            ),
                        )}
                >
                    <div>
                        ...
                        <span class="hidden md:inline"
                            >{$_('Project sennova budgets.singular')}</span
                        >
                    </div>
                </Button>
            {/if}
        ...
        <thead slot="thead">
            <tr class="text-left font-bold">
                ...
                >
                <th class="px-6 pt-6 pb-4 sticky top-0 z-10 bg-white shadow-xl"
                    >Subtotal del costo de los productos o servicios requeridos</th
                >
                <th class="px-6 pt-6 pb-4 sticky top-0 z-10 bg-white shadow-xl"
                    >Acciones</th
                >
        ...
        <tbody slot="tbody">
            {#each projectSennovaBudgets.data as projectSennovaBudget (projectSennovaBudget.id)}
                <tr class="hover:bg-gray-100 focus-within:bg-gray-100">
                    <td class="border-t">
                        <div
                            class="flex flex-col focus:text-indigo-500 px-6 py-4"
                        >
                            <div class="mt-3">
                    ...
                    <td class="border-t">
                        <div class="mt-3">
                            {#if projectSennovaBudget.average > 0}
                                ${new Intl.NumberFormat('de-DE').format(
                                    projectSennovaBudget.average,
                                )} COP
                            {:else if projectSennovaBudget.totalByBudgetWithoutMarketResearch > 0}
                                ${new Intl.NumberFormat('de-DE').format(
                                    projectSennovaBudget.totalByBudgetWithoutMarketResearch,
                                )}
                            {/if}
                        </div>
                        {#if !projectSennovaBudget.call_budget?.sennova_budget?.can_be_added}
                            <span class="text-red-400 text-center text-xs">
                                Este uso presupuestal NO suma al total del
                                presupuesto
                            </span>
                        {/if}
                    </td>
