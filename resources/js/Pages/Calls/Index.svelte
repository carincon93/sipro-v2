			...
			<div>
				{#if activeCall}
					<h1 class="font-bold text-5xl">
						Convocatoria {activeCall.year}
					</h1>
					<p class="text-2xl mt-4">
						La convocatoria empezó el {activeCall.start_date_for_humans}
						y finaliza el {activeCall.end_date_for_humans}.
					</p>
					{#if canIndexCalls || isSuperAdmin}
						<Button
							on:click="{() =>
								Inertia.visit(
									route('calls.dashboard', activeCall.id)
								)}"
							variant="raised"
					...
					{#if canCreateCalls || isSuperAdmin}
						...
						>
							...
							<span class="hidden md:inline"
								>{$_('Calls.singular').toLowerCase()}</span
							>
						</a>
					{/if}
				{/if}
	...
	<div class="py-12">
		{#if canCreateCalls || isSuperAdmin}
			<div class="flex justify-center items-center flex-col">
				...
				<div>
					<Button
						on:click="{() => Inertia.visit(route('calls.create'))}"
						class="mt-8 mb-20"
						variant="raised"
					>
						{$_('Create')}
