<script>
    ...
    let form = useForm({
        ...
        productive_sector_id: {
            value: prioritizedTopic.productive_sector_id,
            label: productiveSectors.find(
                (item) => item.value == prioritizedTopic.productive_sector_id,
            )?.label,
        },
        technical_committee_id: {
            value: prioritizedTopic.technical_committee_id,
            label: technicalCommittees.find(
                (item) => item.value == prioritizedTopic.technical_committee_id,
            )?.label,
        },
    })
    ...
    function destroy() {
        if (canDeletePrioritizedTopics || isSuperAdmin) {
            $form.delete(
                route('prioritized-topics.destroy', prioritizedTopic.id),
            )
        }
    }
</script>
...
<AuthenticatedLayout>
            ...
            <div>
                <h1>
                    {#if canIndexPrioritizedTopics || canShowPrioritizedTopics || canEditPrioritizedTopics || canDeletePrioritizedTopics || isSuperAdmin}
                        <a
                            use:inertia
                            href={route('prioritized-topics.index')}
    ...
    <div class="bg-white rounded shadow max-w-3xl">
        <form on:submit|preventDefault={submit}>
            <fieldset
                class="p-8"
                disabled={canEditPrioritizedTopics || isSuperAdmin
                    ? undefined
                    : true}
            >
                <div class="mt-4">
                ...
                <div class="mt-4">
                    <Label
                        required
                        class="mb-4"
                        labelFor="productive_sector_id"
                        value="Sector productivo"
                    />
                    <Select
                        id="productive_sector_id"
                        items={productiveSectors}
                        bind:selectedValue={$form.productive_sector_id}
                        error={errors.productive_sector_id}
                        autocomplete="off"
                ...
                <div class="mt-4">
                    <Label
                        required
                        class="mb-4"
                        labelFor="technical_committee_id"
                        value="Mesa técnica de servicios tecnológicos"
                    />
                    <Select
                        id="technical_committee_id"
                        items={technicalCommittees}
                        bind:selectedValue={$form.technical_committee_id}
                        error={errors.technical_committee_id}
                        autocomplete="off"
            ...
            >
                {#if canDeletePrioritizedTopics || isSuperAdmin}
                    ...
                    >
                        {$_('Delete')}
                        {$_('Prioritized topics.singular').toLowerCase()}
                    </button>
                {/if}
                {#if canEditPrioritizedTopics || isSuperAdmin}
                    <LoadingButton
