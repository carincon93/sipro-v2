...
Route::middleware(['auth', 'verified'])->group(function () {
    ...
    Route::get('web-api/calls/{call}/{programmaticLine}/project-sennova-roles', function($call, $programmaticLine) {
        ...
        CASE academic_degree
                ...
                WHEN '2' THEN	concat(sennova_roles.name, ' - Nivel académico: Tecnólogo', chr(10), '∙ Asignación mensual: ', call_sennova_roles.salary)
                WHEN '3' THEN	concat(sennova_roles.name, ' - Nivel académico: Pregrado', chr(10), '∙ Asignación mensual: ', call_sennova_roles.salary)
                WHEN '4' THEN	concat(sennova_roles.name, ' - Nivel académico: Especalización', chr(10), '∙ Asignación mensual: ', call_sennova_roles.salary)
                WHEN '5' THEN	concat(sennova_roles.name, ' - Nivel académico: Maestría', chr(10), '∙ Asignación mensual: ', call_sennova_roles.salary)
                WHEN '6' THEN	concat(sennova_roles.name, ' - Nivel académico: Doctorado', chr(10), '∙ Asignación mensual: ', call_sennova_roles.salary)
        END as label,
